...
for i in range(len(distance_ids) ):
    ...
    query = "SELECT id, ST_MemSize(geom) FROM blog_boundaries WHERE id NOT IN \
    (SELECT boundary_id FROM blog_bufferzones WHERE distance_id=%s) order by ST_MemSize(geom)"
    cur.execute(query,[[distance_ids[i][0]])
    ...
    for j in range(len(boundary_ids)):
        ...
        v = cur.execute("""
        INSERT INTO blog_bufferzones(distance_id, boundary_id, geom) 
        ...
        WHERE blog_boundaries.id = %s AND blog_distances.id = %s 
